...
- 支持注册、登录、个人资料管理。
- 注册时要求填写邮箱，并通过验证码完成验证后创建账号。
- 支持通过邮箱重置密码。
- 未配置 SMTP 时，验证码和重置结果会输出到服务端控制台，方便首次完成管理员初始化。
- 支持管理员进行用户管理。
...
首次启动后请访问页面完成注册，由于尚未配置 SMTP，注册验证码会打印到后端控制台，请前往 Docker 容器日志查看并使用验证码完成管理员账号的创建。
...
1. 首次注册一个账号，该账号会自动成为管理员。
2. 完成首次登录后，在后台确认站点名称、描述和菜单图标。
3. 按你的使用场景决定是否开启“强制登录浏览”。
4. 如果面向多人投稿，建议开启“上传审核”。
5. 系统默认允许注册；如果不希望继续开放新用户加入，可在后台关闭“开放注册”。
6. 如果需要正式发送邮箱验证码或找回密码邮件，配置 SMTP 邮件参数；未配置时相关内容会打印到后端控制台。
